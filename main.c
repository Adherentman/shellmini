...
int is_internal_cmd()
{
  ...
  { //dir
    if (cmd_array[1] != NULL && (strcmp(cmd_array[1], ">") == 0 || strcmp(cmd_array[1], ">>") == 0))
    {                   //有重定向，格式是: dir > filename或者dir >> filename
  ...
  else if (strcmp(cmd_array[0], "shift") == 0)
  { //I'll try latter
...
void welcome()
{
  ...
  //为了是程序更友好，加入了颜色
  //颜色是紫色，背景色与shell相同
  printf(CYAN"This is a magic tinyShell\n");
  printf(CYAN"Powered by MuXinYu\n");
  printf(CYAN"Please feel free to experience.\n");
}
...
int is_bg_cmd()
{
  ...
  for (i = 0; i < cmd_cnt && cmd_array[i] != NULL; i++)
  {
  }                //什么都不做，只是为了得到i
  lastpos = i - 1; //最后一个位置
  if (strcmp(cmd_array[lastpos], "&") == 0)
  {                            //命令最末有&
...
void mytest()
{
  ...
  else
  {
    ...
    {                       //如果文件存在
      ...
      mode = buf.st_mode;
      if (strcmp(cmd_array[1], "-l") == 0)
      {                        //如果是符号链接（symbolic link）
      ...
      else if (strcmp(cmd_array[1], "-b") == 0)
      {                        //检查是不是块设备（block device）
      ...
      else if (strcmp(cmd_array[1], "-c") == 0)
      {                        //检查是不是字符设备（character device）
      ...
      else if (strcmp(cmd_array[1], "-d") == 0)
      {                        //检查是不是目录文件（directory）
...
void mybatch()
{
  ...
  { //如果成功
    ...
    for (int i = 0; i < cnt; i++)
    {
      ...
      num = 0;                                //num也要清空
      for (int i = 0; i < MAX_LINE && cmdname[i] != '\0'; i++)
      { //清空cmdname
        cmdname[i] = '\0';
      }
...
void mydir_redirect()
{
  ...
  if (strcmp(cmd_array[1], ">") == 0)
  {
    if ((fd = open(filename, O_CREAT | O_TRUNC | O_WRONLY, 0600)) < 0)
    { //必须用O_TRUNC
      perror("myshell: open");
      exit(1);
    }
  ...
  { //带有>>的重定向，添加内容到文件
    if ((fd = open(filename, O_CREAT | O_APPEND | O_WRONLY, 0600)) < 0)
    { //必须使用O_APPEND
      perror("myshell: open");
      exit(1);
    }
